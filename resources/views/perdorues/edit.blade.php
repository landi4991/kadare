...
@section('content')
 <nav aria-label="breadcrumb">
  <ol class="breadcrumb">
    ...
    <li class="breadcrumb-item active" aria-current="page">Edito përdorues</li>
  </ol>
</nav>
    <div class="row">
        <div class="col-12">
    ...
    <div class="card-body">
        <form action="{{route('perditeso',['user_id' => $users->user_id])}}" role="form" method="POST" enctype="multipart/form-data">
            {!!csrf_field()!!}         
            ...
            <div class="form-group">
                <label class="required" for="roles">Roli</label>
                            ...
                            <?php $selectedvalue=$users['id']?>

                           <select name="id" id="select" class="form-select">

                               @foreach ($roles as $role)
                                <option value="{{$role->id}}"{{$selectedvalue==$role->id?'selected="selected"':''}}>{{$role->role}}</option>
                                    @endforeach
                              </select>
